...
Note: This demo uses an instance of mongodb from mlab.com (free acount with 500 mb of space). Status: [http://status.mlab.com/](http://status.mlab.com/)
And uses https://www.openshift.com/ to host the weatherWeb website (free account for up to 3 instances of an application. They changed there policy on free accounts but mine and other people who are already registered stay the same). And my raspberry pi runs the weatherService and DHTService I wrote to updated mongodb with the hourly outdoor and real-time indoor weather. You can see the architecture was designed to have this run for free :)
